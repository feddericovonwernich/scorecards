...
case "$1 $2" in
    "pr list")
        if [ "${PR_STATE:-NONE}" = NONE ]; then
            printf '%s\n' '[]'
        else
            printf '[{"number":17,"state":"%s","title":"Install Scorecards","headRefName":"scorecards-install-old","url":"https://github.com/acme/service/pull/17"}]\n' "$PR_STATE"
        fi
        ;;
    "label create") ;;
