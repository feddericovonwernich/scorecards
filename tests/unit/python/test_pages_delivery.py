...
class PagesDeliveryTests(unittest.TestCase):
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                if owner.behavior == "redirect":
                    ...
                    self.send_header(
                        "Location",
                        f"https://localhost:{owner.server.server_port}"
                        f"/leak?{SECRET}",
                    )
                    self.end_headers()
                    return
                if owner.behavior == "forbidden" or (
                    owner.behavior == "restricted"
                    and self.headers.get("Cookie") != f"pages_session={SECRET}"
                ):
                ...
                if owner.behavior == "login":
                    content, kind = (
                        b"<html><form>Sign in to GitHub</form></html>",
                        "text/html",
                    )
                elif self.path == "/":
    ...
    def setUp(self):
        ...
        self.cookie_file.write_text(
            ...
            f"127.0.0.1\tFALSE\t/\tTRUE\t{int(time.time()) + 3600}"
            f"\tpages_session\t{SECRET}\n"
        )
    ...
    def invoke(self, auth=False, operation="verify", url=None):
        environment = {
            ...
            if not key.startswith("SCORECARDS_PAGES_")
        }
        environment.update(
            SSL_CERT_FILE=str(self.cert),
            GITHUB_TOKEN="repository-pat-not-pages-auth",
            SCORECARDS_AUTO_CONFIRM="true",
        )
        if auth:
            environment.update(
                SCORECARDS_PAGES_AUTH="browser-session",
                SCORECARDS_PAGES_COOKIE_FILE=str(self.cookie_file),
            )
        args = [sys.executable, str(VERIFIER), operation, url or self.url]
        if operation == "verify":
            args.extend(["1", "0.05"])
        result = subprocess.run(
            args, env=environment, capture_output=True, text=True, timeout=5
        )
        self.assertNotIn(SECRET, result.stdout + result.stderr)
        self.assertNotIn(
            "repository-pat-not-pages-auth", result.stdout + result.stderr
        )
        self.assertTrue(all(
            authorization is None for _, _, authorization in self.requests
        ))
        return result
    ...
    def test_public_delivery_does_not_send_repository_token(self):
        ...
        self.assertEqual(
            {path for path, _, _ in self.requests},
            {"/", "/assets/index-abcdefgh.js", "/assets/index-abcdefgh.css"},
        )
    ...
    def test_restricted_delivery_authenticates_html_and_compiled_assets(self):
        ...
        self.assertTrue(all(
            cookie == f"pages_session={SECRET}"
            for _, cookie, _ in self.requests
        ))
    ...
    def test_expired_session_fails_preflight_without_network(self):
        self.cookie_file.write_text(
            "# Netscape HTTP Cookie File\n"
            f"127.0.0.1\tFALSE\t/\tTRUE\t1\tpages_session\t{SECRET}\n"
        )
        self.assertNotEqual(
            self.invoke(auth=True, operation="preflight").returncode, 0
        )
    ...
    def test_other_host_cookie_fails_preflight(self):
        self.cookie_file.write_text(
            "# Netscape HTTP Cookie File\n"
            f"github.com\tFALSE\t/\tTRUE\t0\tuser_session\t{SECRET}\n"
        )
        self.assertNotEqual(
            self.invoke(auth=True, operation="preflight").returncode, 0
        )
    ...
    def test_session_is_never_sent_over_http(self):
        self.assertNotEqual(
            self.invoke(
                auth=True, url=self.url.replace("https:", "http:")
            ).returncode,
            0,
        )
        self.assertEqual(self.requests, [])
