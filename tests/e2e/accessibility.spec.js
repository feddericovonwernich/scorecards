...
test.describe('Accessibility', () => {
  ...
  test('dashboard and page sections have no critical violations', async ({ page }) => {
    ...
    const headerResults = await new AxeBuilder({ page })
      ...
      .analyze();
    expect(
      headerResults.violations.filter((v) => v.impact === 'critical' || v.impact === 'serious')
    ).toHaveLength(0);
    ...
    expect(
      controlsResults.violations.filter((v) => v.impact === 'critical' || v.impact === 'serious')
    ).toHaveLength(0);
    ...
    expect(
      statsResults.violations.filter((v) => v.impact === 'critical' || v.impact === 'serious')
    ).toHaveLength(0);
  });
  ...
  test('modals are accessible', async ({ page }) => {
    ...
    expect(
      serviceResults.violations.filter((v) => v.impact === 'critical' || v.impact === 'serious')
    ).toHaveLength(0);
    ...
    expect(
      settingsResults.violations.filter((v) => v.impact === 'critical' || v.impact === 'serious')
    ).toHaveLength(0);
  });
  ...
  test('Settings restores focus when authentication removes the nested opener', async ({
    page,
  }) => {
  ...
  test('workflow entry without a token opens Settings without dispatching', async ({ page }) => {
    ...
    await page
      ...
      .getByRole('button', { name: 'Re-run scorecard workflow' })
      .first()
      .click();
  ...
  test('backdrop closes only gestures that start and end outside dialog content', async ({
    page,
  }) => {
