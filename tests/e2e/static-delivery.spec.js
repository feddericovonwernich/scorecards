...
async function mockExplorerRequests(page, service = explorerService) {
  ...
  await page.route('https://unpkg.com/swagger-ui-dist@5.11.0/swagger-ui.css', route =>
    route.fulfill({ status: 200, contentType: 'text/css', path: 'node_modules/swagger-ui-dist/swagger-ui.css' })
  );
...
test.describe('Static catalog delivery', () => {
  ...
  test('loads the compiled Explorer and exposes a mocked operation schema', async ({ page }, testInfo) => {
    ...
    const model = page.locator('.swagger-ui .models').getByRole('button', { name: 'TestResponse', exact: true });
    ...
    for (const [width, height] of [[320, 844], [360, 844], [390, 844], [430, 844], [768, 1024], [1440, 900], [844, 390]]) {
      ...
      measurements.push(await page.evaluate(() => ({ viewport: [innerWidth, innerHeight], documentWidth: document.documentElement.scrollWidth, url: location.href })));
      await page.screenshot({ path: testInfo.outputPath(`explorer-${width}x${height}.png`), fullPage: true });
    }
    await testInfo.attach('Explorer viewport measurements', { body: JSON.stringify(measurements, null, 2), contentType: 'application/json' });
  });
  ...
  test('shows Explorer error states from isolated responses', async ({ page }) => {
    ...
    await expect(page.getByText('Missing parameters. Please specify org and repo in the URL.')).toBeVisible();
