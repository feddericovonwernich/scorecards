...
const responseValue =
  'https://example.invalid/this-is-a-long-response-value-that-must-remain-readable-without-widening-the-document/end-of-response';
...
test('expanded Explorer keeps source, response and schema reachable without document overflow', async ({
  page,
}, testInfo) => {
  await page.route(/^https:\/\//, (route) => {
    ...
    if (url === 'https://unpkg.com/swagger-ui-dist@5.11.0/swagger-ui.css') {
      return route.fulfill({
        ...
        path: 'tests/e2e/fixtures/swagger-ui-5.11.0.css',
      });
    }
    if (new URL(url).pathname.endsWith('/catalog/results/fixture/api/results.json')) {
      return route.fulfill({
        json: {
          service: { name: 'Fixture API', openapi: { spec_file: 'openapi.yaml', branch: 'main' } },
        },
      });
    }
  ...
  for (const property of ['message', 'identifier']) {
    await page
      .locator('.models')
      .getByRole('row', { name: `${property} [...]`, exact: true })
  ...
  for (const [width, height] of [
    ...
    await expect(
      page.getByRole('link', {
        name: 'https://raw.githubusercontent.com/fixture/api/main/openapi.yaml',
        exact: true,
      })
    ).toBeVisible();
    await expect(page.getByRole('tabpanel', { name: 'Example Value' })).toContainText(
      responseValue
    );
    ...
    for (const tab of ['Schema', 'Example Value']) {
      ...
      await testInfo.attach(`dimensions-${tab}-${width}x${height}`, {
        body: JSON.stringify(dimensions),
        contentType: 'application/json',
      });
      expect(dimensions.document).toBe(dimensions.viewport);
      const regions = await page
        .locator('.responses-inner:visible, .model-box:visible')
      ...
      await testInfo.attach(`scroll-regions-${tab}-${width}x${height}`, {
        body: JSON.stringify(regions),
        contentType: 'application/json',
      });
      expect(regions.every((region) => region.endReached)).toBe(true);
      await page.screenshot({
        path: testInfo.outputPath(`explorer-${tab}-${width}x${height}.png`),
        fullPage: true,
      });
    }
  }
