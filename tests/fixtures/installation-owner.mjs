...
const caller = yaml.load(
  await fs.readFile('documentation/examples/service-workflow-example.yml', 'utf8')
).jobs.scorecards;
...
for (const [name, secret] of Object.entries(contract.secrets)) {
  if (secret.required)
    assert.ok(Object.hasOwn(credentials, name), `missing required secret ${name}`);
}
for (const name of Object.keys(caller.secrets))
  assert.ok(Object.hasOwn(contract.secrets, name), `undeclared secret ${name}`);
...
const inputs = {
  org: 'acme',
  repo: 'service',
  'scorecards-repo': 'acme/scorecards',
...
const values = {
  ...inputValues,
  'secrets.scorecards-workflow-token': 'fixture-token',
  'secrets.SCORECARDS_WORKFLOW_TOKEN': 'fixture-token',
};
const run = (command, args, cwd = temp, env = {}) =>
...
let polls = 0;
let releaseFirst;
const bothRequested = new Promise((resolve) => {
...
const step = async (definition, cwd, env, expressions = values) => {
  ...
  const stepEnv = Object.fromEntries(
    Object.entries(definition.env ?? {}).map(([key, value]) => [key, render(value, expressions)])
  );
  const result = await run('bash', ['-eo', 'pipefail', filename], cwd, {
    ...stepEnv,
    ...env,
...
const executeOwner = async (record) => {
  ...
  const env = {
    ...fixtureEnv,
    FIXTURE_RUN: String(record.id),
    GITHUB_REPOSITORY: 'acme/scorecards',
    GITHUB_REF: 'refs/heads/main',
    GITHUB_RUN_ID: String(record.id),
    GITHUB_RUN_ATTEMPT: '1',
    GITHUB_STEP_SUMMARY: path.join(workspace, 'summary'),
  };
  ...
  await fs.copyFile(
    path.join(root, 'documentation/examples/scorecard-workflow-template.yml'),
    path.join(templates, 'scorecard-workflow-template.yml')
  );
  const steps = owner.jobs['create-pr'].steps;
  const check = await step(
    steps.find((s) => s.id === 'check'),
    checkout,
    env
  );
  ...
  if (check.outputs['create-pr'] === 'true') {
    const prepared = await step(
      steps.find((s) => s.id === 'prepare'),
      checkout,
      env
    );
    assert.equal(prepared.code, 0, prepared.output);
    const result = await step(
      steps.find((s) => s.id === 'create-pr'),
      checkout,
      env,
      { ...values, 'steps.prepare.outputs.install-branch': prepared.outputs['install-branch'] }
    );
    assert.equal(result.code, 0, result.output);
  ...
  for (const [prefix, data] of [
    ...
    for (const key of ['status', 'message', 'pr-number', 'pr-state', 'pr-url'])
      resultValues[`steps.${prefix}.outputs.${key}`] = data[key] ?? '';
  }
  const result = await step(
    steps.find((s) => s.id === 'result'),
    checkout,
    env,
    resultValues
  );
  ...
  for (const [key, value] of Object.entries(result.outputs))
    publishValues[`needs.create-pr.outputs.${key}`] = value;
  const published = await step(
    owner.jobs['publish-result'].steps[0],
    workspace,
    env,
    publishValues
  );
  ...
  record.artifactName = render(owner.jobs['publish-result'].steps[1].with.name, {
    'inputs.request-id || github.run_id': record.request,
  });
    ...
    if (args[0] === 'api') {
      ...
      if (mode === 'concurrent' || mode === 'delayed') enqueue(request);
        ...
        runs.push({
          id: nextId++,
          request,
          status: mode === 'timeout' ? 'queued' : 'completed',
          conclusion: mode === 'missing-artifact' ? 'success' : mode,
        });
      ...
      pr = {
        ...
        state: 'OPEN',
        title: 'Install',
        headRefName: arg(args, '--head'),
        url: `https://github.com/acme/service/pull/${18 + creates - 1}`,
      };
      ...
      output = JSON.stringify(
        runs.map((record) => ({
          ...
          displayTitle: render(owner['run-name'], {
            'inputs.request-id || github.run_id': record.request,
          }),
        }))
      );
      ...
      const waiting =
        mode === 'delayed' &&
        polls *
          Number(service.jobs['request-installation-pr'].steps[0].env.POLL_INTERVAL_SECONDS) <=
          420;
      output = JSON.stringify({
        status: waiting ? 'in_progress' : record.status,
        conclusion: waiting ? '' : record.conclusion,
      });
...
try {
  ...
  const polling =
    mode === 'delayed' || mode === 'timeout'
      ? { POLL_INTERVAL_SECONDS: '0' }
      : { POLL_ATTEMPTS: '100', POLL_INTERVAL_SECONDS: '0.05' };
  const result = await step(service.jobs['request-installation-pr'].steps[0], temp, {
    ...fixtureEnv,
  ...
  if (mode === 'concurrent') {
    ...
    assert.equal(
      await git(
        ...
        'for-each-ref',
        '--format=%(refname)',
        'refs/heads/scorecards-install-*'
      ),
      'refs/heads/scorecards-install-100-1'
    );
    const guard = owner.jobs['validate-owner'].steps[0];
    for (const env of [
      { GITHUB_REPOSITORY: 'other/scorecards', GITHUB_REF: 'refs/heads/main' },
      { GITHUB_REPOSITORY: 'acme/scorecards', GITHUB_REF: 'refs/tags/main' },
    ]) {
    ...
    if (mode === 'timeout')
      assert.equal(
        polls,
        Number(service.jobs['request-installation-pr'].steps[0].env.POLL_ATTEMPTS)
      );
  }
} finally {
