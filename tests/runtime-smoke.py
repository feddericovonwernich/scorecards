#!/usr/bin/env python3
"""Exercise a built image without publishing or accessing a service \
repository."""
import json
...
with tempfile.TemporaryDirectory(
    ...
    common = [
        ...
        '--cpus', policy['cpu_max'],
        '--tmpfs',
        f"/tmp:rw,nosuid,nodev,noexec,size={policy['memory_max_bytes']}",
    ...
    def run(
        entry: str, *args: str, writable: bool = False, expected: int = 0,
    ) -> str:
        ...
        assert result.returncode == expected, (
            entry, result.returncode, result.stdout, result.stderr,
        )
        assert (
...
assert int(next(x.split()[1] for x in status \
if x.startswith('CapEff:')), 16) == 0
...
print(json.dumps({'uid': os.getuid(), 'network': 'loopback only', \
'capabilities': 0, 'no_new_privileges': True, 'mounts': 'read-only'}))
''')
    tools = run('/bin/bash', '-euc', '''
for tool in curl wget git jq gpg bash grep sed gawk find sort python \
python3 pip3 gcc g++ make; do
    command -v "$tool"
done
...
cd /action
node --input-type=module -e 'for (const name of \
["@apidevtools/swagger-parser", "js-yaml", "marked", "yaml"]) \
await import(name);'
''')
    ...
    # Exercise scoring's default entrypoint and real ES-module resolution.
    (workspace / 'openapi.json').write_text(json.dumps({
        'openapi': '3.0.0',
        'info': {'title': 'Runtime probe', 'version': '1.0.0'}, 'paths': {},
    }))
    ...
    print(json.dumps({
        'image': image, 'isolation': json.loads(isolation),
        'tools': tools.splitlines(),
        'badge': 'fail -> recipe -> pass', 'idempotent': True,
        'scoring': {r['check_id']: r['status'] for r in results},
    }, indent=2))
