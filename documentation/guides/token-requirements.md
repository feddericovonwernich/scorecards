...
**Solution:**
- For catalog writes: Check `SCORECARDS_CATALOG_TOKEN` has `repo` scope and can access the scorecards repository
- For PR creation: Check `SCORECARDS_WORKFLOW_TOKEN` has both `repo` and `workflow` scopes
...
2. Verify secret exists in organization/repository
3. Confirm repository access permissions
4. Generate new token if needed

### "Bad credentials"

**Cause:** Secret not available to workflow.

**Solution:**
- Verify secret name matches exactly (case-sensitive)
- Check repository has access to organization secret
- Ensure organization allows secrets for private repos (if applicable)

### Installation PR Creation Fails

**Cause:** Missing or incorrect `SCORECARDS_WORKFLOW_TOKEN`.

**Solution:**
- Create `SCORECARDS_WORKFLOW_TOKEN` with `repo` + `workflow` scopes
- Must be a **classic token** (fine-grained doesn't support workflow scope)
- Verify secret is available to the workflow
...
### Using Reusable Install Workflow
