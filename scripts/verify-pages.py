...
class Assets(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if (
            tag == "link"
            and "modulepreload" in attrs.get("rel", "").split()
            and attrs.get("href")
        ):
...
def session_cookies():
    ...
    with open(path, "rb") as stream:
        ...
        if (
            ...
            raise ValueError(
                "Pages cookie file must be owned by you "
    ...
    if not any(cookie.secure and not cookie.is_expired() for cookie in jar):
        raise ValueError(
            "Pages cookie file has no unexpired secure browser session"
        )
    return jar
...
def origin(address):
    ...
    if (
        ...
        raise ValueError(
            "Pages URL must not contain credentials, query, or fragment"
        )
...
def preflight(url=""):
    ...
    if mode not in {"public", "browser-session"}:
        raise ValueError(
            "SCORECARDS_PAGES_AUTH must be public or browser-session"
        )
    if mode == "public":
        if os.environ.get("SCORECARDS_PAGES_COOKIE_FILE"):
            raise ValueError(
                "Pages cookie input requires browser-session mode"
            )
        return None
    jar = session_cookies()
    if jar is None:
        if os.environ.get("SCORECARDS_AUTO_CONFIRM") == "true":
            raise ValueError(
                "Unattended restricted Pages verification requires "
                "SCORECARDS_PAGES_COOKIE_FILE"
            )
        with open("/dev/tty", "r"):
            pass
    if url:
        ...
        if jar is not None and not cookie_header(jar, url):
            raise ValueError(
                "No unexpired host-bound session for the Pages URL; "
                "sign in to that site first"
            )
    return jar
...
def cookie_header(jar, address):
    ...
    for cookie in jar:
        if (
            cookie.secure
            and cookie.domain == parts.hostname
            and not cookie.domain_initial_dot
...
def verify(url, timeout, interval):
    ...
    def expired(*_):
        raise SystemExit(
            f"Pages delivery verification timed out: {last_error}"
        )
    ...
    if mode == "browser-session" and jar is None:
        cookie = getpass.getpass(
            f"Sign in to {url} in your browser, then paste only that site's "
            "Cookie request header (hidden): "
        )
        if not cookie or any(
            ord(char) < 32 or ord(char) > 126 for char in cookie
        ):
    ...
    def fetch(address, kind):
        ...
        with opener.open(
            Request(address, headers=headers), timeout=timeout
        ) as response:
    ...
    while True:
        try:
            ...
            if not assets.module_script or not assets.app_root:
                raise ValueError(
                    "Pages response is not the compiled Scorecards "
                    "application; check browser authentication"
                )
            compiled = {
                (urljoin(url, reference), kind)
                for reference, kind in assets.references
                if urlsplit(urljoin(url, reference)).netloc
                == urlsplit(url).netloc
            }
            if {kind for _, kind in compiled} != {"js", "css"}:
                raise ValueError(
                    "Pages HTML is missing compiled JavaScript or stylesheets"
                )
            for address, kind in sorted(compiled):
                if not re.search(
                    r"/assets/[^/]+-[A-Za-z0-9_-]{8,}\." + kind + r"$",
                    urlsplit(address).path,
                ):
                    raise ValueError("Pages HTML references an unhashed asset")
                fetch(address, kind)
            print(
                f"Verified deployed HTML and {len(compiled)} compiled assets"
            )
            signal.alarm(0)
            return
            ...
            if error.code in {301, 302, 303, 307, 308, 401, 403}:
                raise ValueError(
                    "Pages authentication or redirect refused; "
                    "refresh the site-specific browser session"
                ) from None
            last_error = f"HTTP {error.code}"
        except (OSError, ValueError) as error:
            last_error = (
                "Pages content or session validation failed"
                if isinstance(error, ValueError)
                else "Pages transport failure"
            )
...
def main():
    try:
        ...
        print(
            "Pages verification failed: check the site URL, browser session, "
            "private cookie-file permissions, and authentication mode",
            file=sys.stderr,
        )
        return 1
    return 0
